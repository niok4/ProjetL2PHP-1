<?php
	...
	$TournoiEnGestion = getTournoi((int)strval($_GET['idTournoi']));
...
<html lang="fr">
	...
	<body>
	<?php
		if (!IsAlreadyProgrammed((int)strval($_GET['idTournoi']))){
		...
		if ($TournoiEnGestion->aVenir()){
			...
			echo '<table>
					...
					for($i=0;$i < ($tasTournoi->nbTours() + 1);++$i){
							...
							if(estGestionnaire($ut->getIdUtilisateur()) && !IsAlreadyProgrammed((int)strval($_GET['idTournoi'])))
							{
		...
		if(IsAlreadyProgrammed((int)strval($_GET['idTournoi']))){
			$indexMatchT = 0;
			$tabMachT =  getAllMatchTwithIdT(strval($_GET['idTournoi']));
			$machDansCeTour = count($tabEquipe)/2;
			for($i=1;$i < ($tasTournoi->nbTours() + 1);++$i){
				for($j=0;$j <= $machDansCeTour;++$j){
					echo 'match '.($j+1).' de tour '.$i.' est le '.$tabMachT[$indexMatchT]->getdateMatchT().' a '.$tabMachT[$indexMatchT]->gethoraire().'  ';
					echo '<br>';
